...
1. GUI Slider のパネルを持つアーマチュアを、現在の View Layer に用意する
2. 操作したいプロパティを右クリックする
3. **GUIスライダーを追加** を選ぶ
4. 作成先のコンテナを選び、必要ならスライダー名を入力して **OK** を押す
...
!!! note "表示条件"
    これらの項目は、Blender 標準の右クリックメニューに **Add Driver** が表示されるプロパティでのみ表示される。**リンクGUIスライダーを追加** は、現在の View Layer に既存のGUIスライダーが1つ以上ある場合に表示される。
